    ...
    def test_dashboard_combines_health_flow_and_audit(self):
        with TemporaryDirectory() as folder:
            ...
            with closing(sqlite3.connect(data / "scheduler-status.db")) as connection:
                connection.execute("CREATE TABLE task_runs(mode TEXT PRIMARY KEY,completed_at TEXT,returncode INTEGER)")
                connection.execute("INSERT INTO task_runs VALUES('health','2026-09-09T12:00:00+00:00',0)")
                connection.commit()
            with closing(sqlite3.connect(data / "professional.db")) as connection:
                connection.execute("CREATE TABLE stream_status(id INTEGER PRIMARY KEY,last_message_at INTEGER,reconnects INTEGER,last_error TEXT)")
                connection.execute("CREATE TABLE order_flow_minutes(minute INTEGER,symbol TEXT,best_bid TEXT,best_ask TEXT,spread TEXT,bid_depth TEXT,ask_depth TEXT,imbalance TEXT,buy_notional TEXT,sell_notional TEXT,trade_count INTEGER)")
