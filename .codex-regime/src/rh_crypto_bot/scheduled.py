...
def _calendar_marker(key: str, *, write: bool = False) -> bool:
    ...
    with closing(sqlite3.connect(STATUS_DATABASE)) as connection:
        connection.execute(
            "CREATE TABLE IF NOT EXISTS calendar_notifications (marker TEXT PRIMARY KEY, sent_at TEXT NOT NULL)"
        )
        exists = (
            connection.execute(
                "SELECT 1 FROM calendar_notifications WHERE marker=?", (key,)
            ).fetchone()
            is not None
        )
        if write:
            connection.execute(
                "INSERT OR IGNORE INTO calendar_notifications(marker,sent_at) VALUES(?,?)",
...
def _calendar_message(events: list[dict[str, object]], title: str) -> str:
    ...
    for event in events:
        start = datetime.fromtimestamp(int(event["start"]), timezone.utc).strftime(
            "%Y-%m-%d %H:%M UTC"
        )
...
def _health_message(summary: dict[str, object], runs: dict[str, tuple[str, int]]) -> str:
    ...
    challenger = ShadowStore(challenger_path).summary() if challenger_path.exists() else {}
    challenger_evidence = challenger.get("independent_signal_results", {})
    ...
    return "\n".join(
        [
            ...
            f"• Balanced challenger signals: {challenger_evidence.get('signals', 0)}",
...
def _run_health() -> int:
    ...
    try:
        ...
        if (
            settings
            and settings.discord_health_webhook_url
            and not isinstance(error, DiscordNotificationError)
        ):
            try:
                DiscordWebhook(settings.discord_health_webhook_url).send(
...
def run(mode: str) -> int:
    ...
    try:
        commands = [[sys.executable, "-m", "rh_crypto_bot.cli", command]]
        challenger = [
            sys.executable, "-m", "rh_crypto_bot.cli", command,
            "--shadow-database", "data/shadow-balanced.db",
        ]
        challenger.extend(["--strategy-profile", "balanced"])
        commands.append(challenger)
        results = [
            subprocess.run(
                invocation, cwd=PROJECT_ROOT, env=environment, capture_output=True,
        ...
        output = "\n".join(
            f"track={'strict' if index == 0 else 'balanced'}; exit={result.returncode}"
            f"\nstdout:\n{result.stdout}\nstderr:\n{result.stderr}"
