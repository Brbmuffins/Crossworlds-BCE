...
class DiscordTests(unittest.TestCase):
    ...
    def test_sender_preserves_readable_line_breaks(self) -> None:
        with patch("rh_crypto_bot.discord.urlopen", return_value=_Response()) as send:
            DiscordWebhook("https://discord.com/api/webhooks/1/secret").send(
                "**Heading**\n\n• First item\n• Second item"
            )
        payload = json.loads(send.call_args.args[0].data)
        self.assertEqual(payload["content"], "**Heading**\n\n• First item\n• Second item")
    ...
    def test_tracked_markets_separates_benchmark_and_altcoins(self) -> None:
        ...
        self.assertIn("**Altcoins (2)**\n• ETH\n• SOL", message)
        self.assertIn("**Total:** 3 markets", message)
        self.assertIn("no real orders", message.lower())
    ...
    def test_promotion_message_requires_complete_gate(self) -> None:
        ...
        self.assertIn("**Research Gate Passed**", message)
        self.assertIn("Independent signals: 25", message)
        self.assertIn("Profit factor: 1.80", message)
        self.assertIn("Maximum drawdown: +8.00%", message)
    ...
    def test_action_message_contains_only_tracking_summary(self) -> None:
        ...
        self.assertIn("BTC — Target reached", message)
        self.assertIn("Hypothetical P/L: +$7.00", message)
        self.assertIn("no real orders", message.lower())
