...
from .event_strategy import EventStrategyConfig
...
def _send_shadow_discord(
    ...
    if milestone and not store.notification_sent(PROMOTION_READY_NOTIFICATION):
...
def main() -> None:
    ...
    if args.command == "dashboard":
        ...
        _send_shadow_discord(store, settings.discord_webhook_url, actions, summary)
        ...
        benchmark = universe["BTC-USD"]
        config = EventStrategyConfig(expected_interval_seconds=interval)
        breadth = market_breadth(universe, config)
        latest_breadth = breadth.get(benchmark[-1].start, Decimal("0"))
        leadership = market_leadership(benchmark, universe, latest_breadth)
        try:
        ...
        for symbol, candles in sorted(universe.items()):
            ...
            rationale = (
                f"{latest.reason}; regime={asset_regime.trend}/{asset_regime.phase}; "
