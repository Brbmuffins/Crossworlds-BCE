...
| Abilities | `AbilityCaster` + serializable `AbilityDef` (data-driven: shape, charge, instant-cast delivery, chain, pull, shield, deployable), per-class handlers (`DashHandler`, `StealthHandler`, `SiegeModeHandler`, `IronTetherHandler`, …), deployables (`DeployableManager`, `PhaseRelayDeployable`, `BastionNode`, …), `SnapshotSystem` | Mixed — Commands→server effects; needs per-file review before hardening |
