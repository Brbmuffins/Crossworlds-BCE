...
| Abilities | `AbilityCaster` + serializable `AbilityDef` (data-driven: shape, charge, instant-cast delivery, chain, pull/knock-up CC, shield, deployable), per-class handlers (`DashHandler`, `StealthHandler`, `SiegeModeHandler`, `IronTetherHandler`, …), deployables (`DeployableManager`, `PhaseRelayDeployable`, `BastionNode`, …), `SnapshotSystem` | Mixed — Commands→server effects; needs per-file review before hardening |
...
| Equipment | `LootItemDefinition`, `EquipmentAttachmentProfile`, `LootForgeWindow`, `LootForgeIconRenderer`, `PlayerIdentity.Equipment`, `CharacterSheetUI` | Loot Forge authors slots/stats/wearable prefabs, reusable bone/transform profiles, two-handed rules, and 256px transparent icons; character UI supports right-click/drag equip changes; server validates/persists swaps; Mirror replicates visuals and effective stats |
